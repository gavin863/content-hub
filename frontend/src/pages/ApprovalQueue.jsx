...
export default function ApprovalQueue() {
  ...
  return (
    <div>
      <h1 className="text-lg font-semibold mb-4">Pending approval — {currentBrand.name}</h1>
      {loading ? (
        <p className="text-slate-400 text-sm">Loading…</p>
      ) : items.length === 0 ? (
        <p className="text-slate-400 text-sm">Nothing pending approval 🎉</p>
      ) : (
          ...
          {items.map((item) => (
            <Link key={item.id} to={`/content/${item.id}`} className="flex items-center justify-between px-4 py-3 hover:bg-slate-50">
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5">{item.channel_name} · {item.channel_type} · by {item.author_name}</p>
              </div>
